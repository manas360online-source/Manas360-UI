...
export const SessionRunner: React.FC<SessionRunnerProps> = ({ session, onExit, onComplete }) => {
  ...
  if (isCompletedScreen) {
    return (
      <div className="min-h-screen bg-[#FDFCF8] dark:bg-[#0F172A] flex flex-col items-center justify-center py-12 px-6 animate-fade-in text-center transition-colors duration-500">
        <div className="w-full max-w-2xl bg-white dark:bg-[#1E293B] rounded-[40px] p-12 shadow-xl border border-slate-100 dark:border-slate-800 transition-colors">
          <div className="text-6xl mb-6">🎉</div>
          <h2 className="font-serif text-[2.5rem] text-[#0A3A78] dark:text-white mb-4 transition-colors">Session Complete</h2>
          <p className="text-lg text-slate-500 dark:text-slate-400 mb-10 transition-colors">
            Thank you for completing the <strong>{session.title}</strong>. Your responses have been saved.
          </p>
          <button 
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF8] dark:bg-[#0F172A] flex flex-col items-center py-12 px-6 animate-fade-in transition-colors duration-500">
       {/* Header with Exit */}
      ...
      {/* Question Card */}
      <div className="w-full max-w-2xl bg-white dark:bg-[#1E293B] rounded-[32px] p-8 md:p-12 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.05)] dark:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.4)] border border-slate-100 dark:border-slate-800 min-h-[400px] flex flex-col justify-between transition-colors duration-500">
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${getAnswerValue() === opt.id
                        ...
                        : 'bg-[#F1F4F6] dark:bg-slate-800/60 text-[#1A1A1A] dark:text-slate-300 border-[#D5D9DD] dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800'
                      }
                    `}
                  >
                    {opt.label}
            ...
            {currentQuestion.type === 'slider' && (
              <div className="py-8 px-2">
                <input 
                  ...
                  className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-[#1FA2DE] dark:accent-sky-500"
                />
                <div className="flex justify-between mt-4 text-slate-500 dark:text-slate-400 font-medium transition-colors">
                  <span>{currentQuestion.minLabel || 'Low'}</span>
                  <span className="text-[#1FA2DE] dark:text-sky-400 font-bold text-xl transition-colors">{getAnswerValue() || 0}</span>
        ...
        <div className="flex justify-between mt-12 pt-6 border-t border-slate-100 dark:border-slate-700 transition-colors">
           ...
           <button 
             ...
             `}
           >
             {session.questions.findIndex(q => q.id === currentQuestion.id) === session.questions.length - 1 ? 'Finish Session' : 'Next Question'}
           </button>
        </div>
