...
export const AssessmentDashboard: React.FC<AssessmentDashboardProps> = ({ onStartSession, onViewHistory, onBack }) => {
  ...
  const handleDeleteHistory = (id: string) => {
    ...
    if (window.confirm('Are you sure you want to delete this session record? This cannot be undone.')) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF8] dark:bg-[#0F172A] p-8 animate-fade-in transition-colors duration-500">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
          <div>
            ...
            <h1 className="font-serif text-[2.5rem] text-[#0A3A78] dark:text-white font-bold transition-colors">Health Assessment Dashboard</h1>
            <p className="text-slate-500 dark:text-slate-400 transition-colors">Select an assessment to begin your check-in.</p>
          </div>
          ...
          <div className="bg-white dark:bg-[#1E293B] rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors">
            {history.length === 0 ? (
              <div className="p-12 text-center text-slate-400 dark:text-slate-500">
                No completed sessions found. Start an assessment to see results here.
              </div>
            ) : (
                  ...
                  <div key={record.id} className="p-6 flex flex-col md:flex-row items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                        Completed on {new Date(record.completedAt).toLocaleDateString()} at {new Date(record.completedAt).toLocaleTimeString()}
                      </p>
                    </div>
